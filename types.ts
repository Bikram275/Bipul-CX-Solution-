...
export interface FlowStep {
  id: string;
  message: string; // Bot message, supports HTML and {{UNIQUE_ID}} placeholder
  options: FlowOption[];
  isFinal?: boolean; // If true, shows "Final Solution" badge
}
...
export interface SOP {
  ...
  flowSteps?: FlowStep[]; // Optional interactive flow data
  tags: string[];
  lastUpdated: string;
  views: number;
}
...
// Helper to auto-categorize based on title keywords
const determineCategory = (title: string): string => {
    const lowerTitle = title.toLowerCase();
    
    if (lowerTitle.includes('exchange')) return 'Exchange';
    if (lowerTitle.includes('return') || lowerTitle.includes('refund') || lowerTitle.includes('reverse') || lowerTitle.includes('rto')) return 'Return';
    if (lowerTitle.includes('cancel')) return 'Cancelled';
    if (lowerTitle.includes('deliver') || lowerTitle.includes('pod') || lowerTitle.includes('courier')) return 'Delivered';
    if (lowerTitle.includes('dispatch') || lowerTitle.includes('ship') || lowerTitle.includes('awb') || lowerTitle.includes('track') || lowerTitle.includes('transit')) return 'Shipped';
    if (lowerTitle.includes('order') || lowerTitle.includes('place_order') || lowerTitle.includes('checkout')) return 'Ordered';
    
    return 'Non Order';
};

// Map the raw list to initial SOP objects with auto-categorization
export const INITIAL_SOPS: SOP[] = ALL_QUERIES_LIST.map((query) => {
    let flowSteps: FlowStep[] | undefined = undefined;

    // Define interactive flows for specific queries
    ...
    } else if (query === "Delay_delivery_After_TAT") {
        flowSteps = [
            {
                id: "START",
                message: "Is the order status 'Out for Delivery'?",
                options: [
                    { label: "Yes (Out for Delivery)", nextStepId: "OFD_SOLUTION" },
                    { label: "No", nextStepId: "CHECK_COMPLAINT" }
                ]
            },
            {
                id: "OFD_SOLUTION",
                message: "অর্ডারটি আজ ডেলিভারির জন্য বের হয়েছে। দয়া করে সন্ধ্যা পর্যন্ত অপেক্ষা করুন। আজই ডেলিভারি সম্পন্ন হবে বলে আশা করছি। <br><br> <span class='bold'>Unique ID: {{UNIQUE_ID}}</span>",
                options: [],
                isFinal: true
            },
            {
                id: "CHECK_COMPLAINT",
                message: "Is there an existing complaint?",
                options: [
                    { label: "Yes (Existing Complaint)", nextStepId: "EXISTS_SOLUTION" },
                    { label: "No", nextStepId: "NEW_COMPLAINT_SOLUTION" }
                ]
            },
            {
                id: "EXISTS_SOLUTION",
                message: "আপনার পূর্ববর্তী অভিযোগটি আমাদের কাছে আছে। আমরা লজিস্টিকস টিমের সাথে ফলো-আপ করছি এবং দ্রুত সমাধানের চেষ্টা করছি। <br><br> <span class='bold'>Unique ID: {{UNIQUE_ID}}</span>",
                options: [],
                isFinal: true
            },
            {
                id: "NEW_COMPLAINT_SOLUTION",
                message: "ডেলিভারিতে বিলম্বের জন্য দুঃখিত। আমরা অভিযোগটি নথিভুক্ত করেছি। ৪৮ ঘণ্টার মধ্যে আপনার সমস্যার সমাধান করা হবে। <br><br> <span class='bold'>Unique ID: {{UNIQUE_ID}}</span>",
                options: [],
                isFinal: true
            }
        ];
    ...
    return {
        ...
        content: `Generic content for ${query}`, // Fallback content
        flowSteps: flowSteps, // Attach flow steps if defined
        tags: [],
