...
const SOPEditor: React.FC<SOPEditorProps> = ({ initialData, onSave, onCancel }) => {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (initialData.flowSteps && initialData.flowSteps.length > 0) {
        ...
        // Initialize new flow with a START step
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newSOP: SOP = {
      ...
      title,
      category,
      content: mode === 'TEXT' ? content : '', // Only save content if in text mode
      flowSteps: mode === 'FLOW' ? flowSteps : undefined, // Only save flow if in flow mode
    ...
    <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        ...
        <div>
            ...
            <div className="flex gap-4 mb-6">
                <button
                    ...
                    className={`flex-1 py-3 px-4 rounded-lg border flex items-center justify-center gap-2 transition-all ${mode === 'TEXT' ? 'bg-blue-50 border-blue-500 text-blue-700 shadow-sm' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >
                    <Type size={18} />
                    <span className="font-medium">Standard Text / Markdown</span>
                </button>
                <button
                    type="button"
                    onClick={() => setMode('FLOW')}
                    className={`flex-1 py-3 px-4 rounded-lg border flex items-center justify-center gap-2 transition-all ${mode === 'FLOW' ? 'bg-blue-50 border-blue-500 text-blue-700 shadow-sm' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                >
                    <Layers size={18} />
                    <span className="font-medium">Interactive Flow (Chat)</span>
                </button>
            </div>
            ...
            {mode === 'TEXT' ? (
                <div>
                    ...
                    <textarea
                        required={mode === 'TEXT'}
                        className="w-full h-96 px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 outline-none font-mono text-sm"
                        value={content}
                ...
                <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <button 
                            type="button"
                            onClick={addStep}
                            className="text-xs flex items-center gap-1 bg-blue-600 text-white px-3 py-1.5 rounded hover:bg-blue-700"
                        >
                            ...
                            <div key={index} className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 relative">
                                <div className="flex justify-between items-start mb-3">
                                    <div className="flex items-center gap-2">
                                        <div className="bg-slate-800 text-white text-xs px-2 py-0.5 rounded font-mono">
                                            ID: <input 
                                                type="text" 
                                                value={step.id}
                                                onChange={(e) => updateStep(step.id, 'id', e.target.value)}
                                                className="bg-transparent border-b border-gray-500 text-white w-24 outline-none focus:border-white"
                                            />
                                        </div>
                                ...
                                <div className="mb-4">
                                    ...
                                    <textarea
                                        ...
                                        placeholder="Enter the message the bot will say..."
                                    />
                                    <p className="text-[10px] text-gray-400 mt-1">Tip: Use <code>{'{{UNIQUE_ID}}'}</code> to generate a random ID.</p>
                                </div>
                                    ...
                                    <div className="space-y-2">
                                        {step.options.map((opt, optIdx) => (
                                            <div key={optIdx} className="flex items-center gap-2">
                                                <input
                                                    ...
                                                    className="flex-1 text-sm px-2 py-1.5 border border-gray-300 rounded focus:border-blue-500 outline-none"
                                                    value={opt.label}
                                                    onChange={(e) => updateOption(step.id, optIdx, 'label', e.target.value)}
                                                />
                                                <ArrowRight size={14} className="text-gray-400" />
                                                <select
                                                    className="flex-1 text-sm px-2 py-1.5 border border-gray-300 rounded focus:border-blue-500 outline-none bg-white"
                                                    value={opt.nextStepId}
                                                    onChange={(e) => updateOption(step.id, optIdx, 'nextStepId', e.target.value)}
                                                >
                                                    <option value="ENDED">End Chat (End)</option>
                                                    {flowSteps.map(s => (
                                                        <option key={s.id} value={s.id}>{s.id} {s.id === step.id ? '(Self)' : ''}</option>
                                                    ))}
                                                </select>
                                                <button 
                                                    type="button"
                                                    onClick={() => removeOption(step.id, optIdx)}
                                                    className="text-gray-400 hover:text-red-500"
                                                >
                                                    <X size={14} />
                                                </button>
                                        ...
                                        {step.options.length === 0 && (
                                            <div className="text-xs text-gray-400 italic bg-gray-50 p-2 rounded text-center">
                                                No options defined. Flow will end here or auto-complete if Is Final is checked.
                                            </div>
                                        )}
                                    </div>
                                </div>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
            ...
            <button
                type="submit"
                className="flex items-center gap-2 px-6 py-2.5 rounded-lg bg-blue-600 text-white font-medium hover:bg-blue-700 shadow-sm"
            >
                <Save size={18} />
                Save SOP
            </button>
